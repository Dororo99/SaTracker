"""
SatMapTracker Stage 2: Tracking Warmup

Camera BEV only → vector head warmup. Standard MapTracker behavior.
Satellite encoder is loaded from Stage 1 but stays idle (no fusion yet).
BEV backbone frozen, vector head + tracking module trained.
"""
...
model = dict(
    ...
    skip_vector_head=False,     # Vector head active
    freeze_bev=True,            # Camera BEV frozen
    track_fp_aug=False,
    use_memory=True,
    mem_len=4,
    mem_warmup_iters=500,
    # ── Satellite: loaded but idle ──
    use_sat_fusion=False,       # No fusion yet
    freeze_sat_encoder=True,    # Satellite encoder frozen
    ...
    # sat_seg_cfg omitted: no satellite seg head in Stage 2
    # sat_fusion_cfg omitted: no fusion in Stage 2
    backbone_cfg=dict(
    ...
    model_name='SatMapTracker_Stage2',
)

# ── No satellite in pipeline for Stage 2 ──
train_pipeline = [
...
optimizer = dict(
    type='AdamW', lr=5e-4, weight_decay=1e-2)
...
load_from = "work_dirs/satmaptracker_stage1_bev_pretrain/latest.pth"
